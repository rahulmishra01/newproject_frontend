...
const VideoCall = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    socket.current = io("https://new-omagle.onrender.com");
    socket.current.on("ring", ({ from, signalData }) => {
  ...
  const startCall = () => {
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        socket.current.emit("callUser", { from: peerId, to: remotePeerId, signalData: outgoingCall.signal });
      });
  };
  ...
  const answerCall = () => {
    navigator.mediaDevices
      ...
      .then((stream) => {
        localVideoRef.current.srcObject = stream;
        const incomingCall = peerRef.current.call(remotePeerId, stream);
        incomingCall.answer(stream);
        incomingCall.on("stream", (remoteStream) => {
          remoteVideoRef.current.srcObject = remoteStream;
        });
        ...
        socket.current.emit("answerCall", { to: remotePeerId, signal: incomingCall.signal });
      });
  };
