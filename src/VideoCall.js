...
//   const [remotePeerId, setRemotePeerId] = useState("");
//   console.log("🚀 ~ VideoCall ~ remotePeerId:", remotePeerId)
//   const [call, setCall] = useState(null);
//   const [muted, setMuted] = useState(false);
//   const [videoOff, setVideoOff] = useState(false);
//   const [incomingCall, setIncomingCall] = useState(null);
...
const VideoCall = ({ userId, socket }) => {
  ...
  useEffect(() => {
    ...
    socket.on("userJoined", ([id1, id2]) => {
      const remoteId = id1 === userId ? id2 : id1;
  ...
  const startCall = (remoteId) => {
    navigator.mediaDevices
      ...
      .then((stream) => {
        localVideoRef.current.srcObject = stream;
        const outgoingCall = peerRef.current.call(remoteId, stream);
        outgoingCall.on("stream", (remoteStream) => {
          remoteVideoRef.current.srcObject = remoteStream;
        });
        setCall(outgoingCall);
  ...
  const endCall = () => {
    if (call) {
      ...
      localVideoRef.current.srcObject
        .getTracks()
        .forEach((track) => track.stop());
      socket.emit("endCall", { to: remotePeerId });
    }
  };
